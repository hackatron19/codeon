...
with open("Scraped-Data/dataset_uncleaned.csv", encoding="utf-8", errors="ignore") as csvfile:
...
def prediction(disease_pred,disease_real):

    for i in range(0, len(disease_real)):
        if disease_pred[i]!=disease_real[i]:
            return ('Pred: {0} Actual:{1}'.format(disease_pred[i], disease_real[i]))
